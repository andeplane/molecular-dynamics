...
#include <potentials/uscwaterpotential.h>
#include <unitconverter.h>
#include <cmath>
#include <includes.h>
using CompPhys::Utils::at;

void USCWaterPotential::calculateForces(AtomManager &atomManager)
{
    m_potentialEnergy = 0;
...
void USCWaterPotential::twoParticleAction(Atom *atom1, Atom *atom2)
{
...
void USCWaterPotential::threeParticleAction(Atom *atomi, Atom *atomj, Atom *atomk)
{
...
std::string USCWaterPotential::coefficientString() const
{
    char output[10000];
    sprintf(output, "                     Si                     O                          \n");
...
int USCWaterPotential::numberOfPrecomputedTwoParticleForces() const
{
    return m_numberOfPrecomputedTwoParticleForces;
}

void USCWaterPotential::setNumberOfPrecomputedTwoParticleForces(int numberOfPrecomputedTwoParticleForces)
{
    m_numberOfPrecomputedTwoParticleForces = numberOfPrecomputedTwoParticleForces;
    calculatePrecomputedTwoParticleForces();
}

void USCWaterPotential::initialize() {
...
void USCWaterPotential::calculatePrecomputedTwoParticleForces()
{
...
USCWaterPotential::USCWaterPotential()
{
    setName("USC SiO2");
    using namespace std::placeholders;
    m_iteratorDefault.setTwoParticleAction(std::bind(&USCWaterPotential::twoParticleAction, this, _1, _2));
    m_iteratorDefault.setThreeParticleAction(std::bind(&USCWaterPotential::threeParticleAction, this, _1, _2, _3));

    m_iteratorAllPairs.setTwoParticleAction(std::bind(&USCWaterPotential::twoParticleAction, this, _1, _2));
    m_iteratorAllPairs.setThreeParticleAction(std::bind(&USCWaterPotential::threeParticleAction, this, _1, _2, _3));

    initialize();
    setNumberOfPrecomputedTwoParticleForces(8192); // Default value
}

std::ostream& operator<<(std::ostream &stream, const USCWaterPotential &potential) {
