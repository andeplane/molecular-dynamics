...
void Mts0IO::readMts(char *filename, System &system) {
    ...
    systemLength.at(0) = hMatrix[0][0][0];;
    systemLength.at(1) = hMatrix[0][1][1];
    systemLength.at(2) = hMatrix[0][2][2];
    ...
    for(int i=0;i<numberOfAtoms;i++) {
        ...
        double x = (phaseSpace[3*i+0] + m_nodeOrigin[0])*systemLength[0]; // Add node origin since all positions are local on each cpu
        double y = (phaseSpace[3*i+1] + m_nodeOrigin[1])*systemLength[1]; // Also multiply by system length because all positions are between 0 and 1 [1 beeing the maximum coordinate if one processor]
        double z = (phaseSpace[3*i+2] + m_nodeOrigin[2])*systemLength[2];
...
vector<Atom> Mts0IO::loadMts0(string mts0Directory, vector<int> numberOfCPUs, System &system)
{
    vector<Atom> vec;
    ...
    int nx = numberOfCPUs.at(0);
    int ny = numberOfCPUs.at(1);
    int nz = numberOfCPUs.at(2);
